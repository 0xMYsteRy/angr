...
bool State::log_write(address_t address, int size, int clean, bool is_write_symbolic) {
	mem_access_t record;
...
taint_t* State::page_lookup(address_t address) const {
	/*
...
void State::page_activate(address_t address, uint8_t *taint, uint64_t taint_offset) {
	address &= ~0xFFFULL;
	taint_t *bitmap = NULL;
	auto it = active_pages.find(address);
	if (it == active_pages.end()) {
...
instr_details_t State::compute_instr_details(address_t instr_addr, const instruction_taint_entry_t &instr_taint_entry) const {
	instr_details_t instr_details;
